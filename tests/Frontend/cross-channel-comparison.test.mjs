...
test('cross-channel comparison exposes an ordered channel-first two-or-three selection', () => {
    assert.match(page, /Select two or three channels/i);
    ...
    assert.match(page, /selectedIds\.length ===[\s\S]*3/);
    ...
    assert.match(
        types,
        /AnalyzerComparisonRun,[\s\S]*AnalyzerComparisonRun,[\s\S]*AnalyzerComparisonRun/,
    );
});
